...
async function getArticleData(doi: string) {
  try {
    const res = await fetch(`https://api.openalex.org/works/https://doi.org/${doi}?select=title,authorships,publication_date,doi,primary_location,is_retracted,cited_by_count`, {
...
async function getJournalByIssn(issn: string) {
...
export default async function ArticlePage({ searchParams }: { searchParams: Promise<{ doi?: string }> }) {
  ...
  let journal = null
  if (article?.primary_location?.source?.issn) {
    const issn = article.primary_location.source.issn[0]
    journal = await getJournalByIssn(issn)
  }
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-4">
                ...
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    {journal.issn && <p className="text-sm font-mono" style={{ color: '#007A44' }}>{journal.issn}</p>}
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="p-3 rounded-xl text-center" style={{ background: '#F8FAFC' }}>
                    ...
                    <div className="text-xl font-bold text-blue-600">{journal.h_index ?? '—'}</div>
                  ...
                  <div className="p-3 rounded-xl text-center" style={{ background: '#F8FAFC' }}>
                    ...
                    <div className="text-xl font-bold text-purple-600">{journal.total_cites?.toLocaleString() ?? '—'}</div>
